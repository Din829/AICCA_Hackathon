...
class DatabaseToolScheduler:
        ...
        for tool_call in self.tool_calls:
            if tool_call.status == 'scheduled':
                ...
                try:
                    ...
                    # 調用工具結果回調（最小侵入性）
                    if self.on_tool_result and result:
                        try:
                            # 檢查回調是否為異步函數
                            import asyncio
                            import inspect
                            
                            # 為前端準備包含原始參數的數據（不修改原始result對象）
                            result_for_frontend = {
                                "llm_content": result.llm_content,
                                "return_display": result.return_display,
                                "summary": result.summary,
                                "original_args": tool_call.request.args  # 包含原始參數
                            }
                            
                            if inspect.iscoroutinefunction(self.on_tool_result):
                            ...
                            log_info("Scheduler", f"Called on_tool_result for {tool_call.request.name}")
                        except Exception as e:
                            log_info("Scheduler", f"Error calling on_tool_result: {e}")
    ...
    def _create_output_updater(self, call_id: str):
        ...
        def update_output(output: str):
            # 原始的 output_update_handler 用于流式输出
            # 不要在这里调用新的工具结果回调，避免参数不匹配
            pass
