...
class FileReadTool(DatabaseTool):
    ...
    def __init__(self, config: DatabaseConfig, i18n=None):
        ...
        super().__init__(
            ...
            description="[TEMPORARILY DISABLED - DO NOT USE] This tool is temporarily disabled for AICCA. Please do not call this tool. Reads files.",
            parameter_schema={
                "type": "object",
                "properties": {
                    "path": {
    ...
    def validate_tool_params(self, params: Dict[str, Any]) -> Optional[str]:
        ...
        # ===== Web适配：自动处理文件ID =====
        # 检测是否为Web上传的文件ID（UUID格式）
        if self._is_web_file_id(path):
            # 尝试解析文件ID为实际路径
            actual_path = self._resolve_web_file_path(path)
            if actual_path:
                # 透明替换路径，后续逻辑无需修改
                params["path"] = actual_path
                log_info("FileReadTool", f"Resolved file ID {path} to {actual_path}")
                return None  # 验证通过
            else:
                return self._('file_read_invalid_file_id', default="Invalid or expired file ID: {id}", id=path)
        
        # 原有逻辑保持不变：必须是绝对路径（借鉴Gemini CLI）
    ...
    def _is_web_file_id(self, path: str) -> bool:
        """检测是否为Web上传的文件ID（UUID格式）
        
        支持灵活配置：
        - 通过配置禁用：config.file_read_accept_ids = False
        - 自动检测UUID格式
        """
        # 可通过配置完全禁用此功能
        if not self.config.get("file_read_accept_ids", True):
            return False
            
        import re
        # 标准UUID格式：xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
        uuid_pattern = r'^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$'
        return bool(re.match(uuid_pattern, path.lower()))
    
    def _resolve_web_file_path(self, file_id: str) -> Optional[str]:
        """将文件ID解析为实际路径
        
        尝试多种方式，保证灵活性：
        1. 优先使用ContentLoader（如果可用）
        2. 降级到直接查找临时目录
        3. 查找配置的上传目录
        """
        import tempfile
        
        # 方式1：尝试使用ContentLoader（最灵活）
        try:
            # 动态导入，避免硬依赖
            from aicca.utils.content_loader import ContentLoader
            loader = ContentLoader()
            # ContentLoader返回(file_path, metadata)
            import asyncio
            file_path, _ = asyncio.run(loader.load_content(file_id))
            if file_path and os.path.exists(file_path):
                return file_path
        except ImportError:
            # ContentLoader不可用，使用降级方案
            pass
        except Exception as e:
            log_info("FileReadTool", f"ContentLoader failed for {file_id}: {e}")
        
        # 方式2：直接查找标准临时目录
        temp_locations = [
            Path(tempfile.gettempdir()) / "aicca_uploads" / file_id,
            Path(tempfile.gettempdir()) / "aicca_temp" / file_id,
        ]
        
        # 方式3：从配置获取自定义上传目录
        custom_upload_dir = self.config.get("web_upload_directory")
        if custom_upload_dir:
            temp_locations.append(Path(custom_upload_dir) / file_id)
