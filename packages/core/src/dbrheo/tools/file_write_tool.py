...
class FileWriteTool(DatabaseTool):
    ...
    def __init__(self, config: DatabaseConfig, i18n=None):
        ...
        super().__init__(
            ...
            description="[TEMPORARILY DISABLED - DO NOT USE] This tool is temporarily disabled for AICCA. Please do not call this tool. Writes files.",
            parameter_schema={
                "type": "object",
                "properties": {
                    "path": {
    ...
    async def execute(
        ...
        try:
            ...
            # ===== Web适配：可选的下载链接生成 =====
            # 检测是否需要生成下载链接（通过配置或参数控制）
            if self._should_generate_download_link(params):
                download_info = self._create_download_link(path, operation_result)
                if download_info:
                    # 将下载信息添加到结果中，不影响原有结构
                    if not hasattr(operation_result, 'metadata'):
                        operation_result.metadata = {}
                    operation_result.metadata.update(download_info)
                    log_info("FileWriteTool", f"Generated download link: {download_info.get('download_url')}")
...
- Duration: {result.duration_ms:.1f}ms"""
    ...
    def _should_generate_download_link(self, params: Dict[str, Any]) -> bool:
        """判断是否需要生成下载链接
        ...
        """
        # 方式1：参数明确要求生成下载链接
        if params.get("generate_download", False):
            return True
        
        # 方式2：参数明确禁止生成下载链接
        if params.get("no_download", False):
            return False
        
        # 方式3：检测是否来自Web请求（通过特殊标记）
        if params.get("source") == "web" or params.get("web_request", False):
            return True
        
        # 方式4：通过配置控制（默认Web环境启用）
        runtime_env = self.config.get("runtime_env", "cli")
        if runtime_env == "web":
            # Web环境默认生成，除非配置明确禁用
            return self.config.get("file_write_generate_downloads", True)
        
        # CLI环境默认不生成下载链接
        return False
    
    def _create_download_link(self, file_path: Path, result: FileOperationResult) -> Optional[Dict[str, Any]]:
        """创建文件下载链接
        ...
        try:
            ...
            # 获取下载目录（可配置）
            download_base = self.config.get("web_download_directory")
            if not download_base:
                # 默认使用临时目录
                download_base = Path(tempfile.gettempdir()) / "aicca_downloads"
            else:
            ...
            download_path = download_base / f"{download_id}_{safe_filename}"
            
            # 复制文件到下载目录
            shutil.copy2(file_path, download_path)
            
            # 计算过期时间（可配置）
            expiry_seconds = self.config.get("download_expiry_seconds", 3600)  # 默认1小时
            expires_at = datetime.now() + timedelta(seconds=expiry_seconds)
            
            # 构建下载URL（根据实际Web服务器配置）
            base_url = self.config.get("api_base_url", "/api")
            download_url = f"{base_url}/download/{download_id}/{safe_filename}"
            ...
            # 可选：将下载信息存储到缓存或数据库
            # self._store_download_info(download_id, download_info)
            
            return download_info
            
        except Exception as e:
            # 生成下载链接失败不应影响主流程
            log_info("FileWriteTool", f"Failed to create download link: {e}")
            return None
