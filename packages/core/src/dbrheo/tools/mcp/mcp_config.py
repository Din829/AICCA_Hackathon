...
class MCPConfig:
    ...
    def _load_from_runtime(self) -> Optional[Dict[str, Any]]:
        ...
        # 修复：基础DatabaseConfig没有get_test_config方法
        test_config = getattr(self.db_config, 'get_test_config', lambda x: None)('mcp_servers')
        if test_config:
            runtime_config = test_config
        
        # Check for mcp_server_command (single server mode)
        mcp_command = getattr(self.db_config, 'get_test_config', lambda x: None)('mcp_server_command')
