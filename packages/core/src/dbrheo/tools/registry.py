...
class DatabaseToolRegistry:
    ...
    def _register_core_tools(self):
        """注册核心工具 - MVP阶段的基础工具集"""
#         from .sql_tool import SQLTool  # REMOVED: Database tool
#         from .schema_discovery import SchemaDiscoveryTool  # REMOVED: Database tool
        from .file_read_tool import FileReadTool
        from .file_write_tool import FileWriteTool
        from .web_search_tool import WebSearchTool
        from .web_fetch_tool import WebFetchTool
        from .directory_list_tool import DirectoryListTool
#         from .table_details_tool import GetTableDetailsTool  # REMOVED: Database tool
#         from .database_export_tool import DatabaseExportTool  # REMOVED: Database tool
        from .code_execution_tool import CodeExecutionTool
        from .shell_tool import ShellTool
        
        # AICCA内容信任工具导入
        # 注册AICCA内容信任工具
        aicca_tools_registered = 0
        try:
            # 尝试导入AICCA特定工具
            import sys
            from pathlib import Path
            # 添加aicca路径
            aicca_path = Path(__file__).parent.parent.parent / 'aicca' / 'tools'
            if aicca_path.exists():
                # 1. 图像验证工具
                try:
                    from aicca.tools.image_verify_tool import ImageVerifyTool
                    image_verify_tool = ImageVerifyTool(self.config, i18n=self._i18n)
                    self.register_tool(
                        tool=image_verify_tool,
                        capabilities={
                            ToolCapability.ANALYZE,
                            ToolCapability.AUDIT,
                            ToolCapability.EXTERNAL
                        },
                        tags={"image", "verify", "trust", "deepfake", "content", "core", "aicca"},
                        priority=90  # 高优先级 - AICCA核心功能
                    )
                    aicca_tools_registered += 1
                except Exception as e:
                    print(f"[WARNING] Failed to register ImageVerifyTool: {e}")
                
                # 2. C2PA凭证管理工具
                try:
                    from aicca.tools.c2pa_credential_tool import C2PACredentialTool
                    c2pa_tool = C2PACredentialTool(self.config, i18n=self._i18n)
                    self.register_tool(
                        tool=c2pa_tool,
                        capabilities={
                            ToolCapability.ANALYZE,
                        ...
                        tags={"c2pa", "credential", "provenance", "authenticity", "aicca"},
                        priority=85
                    )
                    aicca_tools_registered += 1
                except Exception as e:
                    print(f"[WARNING] Failed to register C2PACredentialTool: {e}")
                
                # 3. AI内容检测工具
                try:
                    from aicca.tools.ai_content_detector import AIContentDetector
                    ai_detector = AIContentDetector(self.config, i18n=self._i18n)
                    self.register_tool(
                        tool=ai_detector,
                        capabilities={
                            ToolCapability.ANALYZE,
                        ...
                        tags={"ai_detection", "gpt", "claude", "synthid", "watermark", "aicca"},
                        priority=87
                    )
                    aicca_tools_registered += 1
                except Exception as e:
                    print(f"[WARNING] Failed to register AIContentDetector: {e}")
                
                # 4. Deepfake检测工具
                try:
                    from aicca.tools.deepfake_detector import DeepfakeDetector
                    deepfake_detector = DeepfakeDetector(self.config, i18n=self._i18n)
                    self.register_tool(
                        ...
                        tags={"deepfake", "face_swap", "voice_clone", "forensics", "aicca"},
                        priority=88
                    )
                    aicca_tools_registered += 1
                except Exception as e:
                    print(f"[WARNING] Failed to register DeepfakeDetector: {e}")
                
                # 5. 合规报告生成工具
                try:
                    from aicca.tools.compliance_report_tool import ComplianceReportTool
                    compliance_tool = ComplianceReportTool(self.config, i18n=self._i18n)
                    self.register_tool(
                        ...
                        tags={"compliance", "eu_ai_act", "regulations", "audit", "report", "aicca"},
                        priority=80
                    )
                    aicca_tools_registered += 1
                except Exception as e:
                    print(f"[WARNING] Failed to register ComplianceReportTool: {e}")
                
                if aicca_tools_registered > 0:
                    print(f"[INFO] Successfully registered {aicca_tools_registered} AICCA content trust tools")
                
        except Exception as e:
            print(f"[ERROR] Failed to load AICCA tools: {e}")
        
        # [REMOVED: SQL和Schema工具 - AICCA不需要数据库功能]
        ...
        # [REMOVED: 表详情和数据导出工具 - AICCA不需要数据库功能]
        ...
        self.register_tool(
            ...
            capabilities={
                ...
                # ToolCapability.BACKUP,    # 备份功能（AICCA不需要）
                # ToolCapability.RESTORE,   # 恢复功能（AICCA不需要）
                ToolCapability.AUDIT      # 系统审计
            },
            tags={"shell", "system", "management", "monitoring", "core"},
            priority=85  # 高优先级工具，系统管理
        )
        
        # [REMOVED: 数据库连接工具 - AICCA不需要数据库功能]
