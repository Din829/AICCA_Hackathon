...
def convert_to_function_response(
    ...
    # 多重检测策略，确保ToolResult对象被正确识别
    is_tool_result = False
    
    # 策略1: 通过类名检测
    if hasattr(llm_content, '__class__') and llm_content.__class__.__name__ == 'ToolResult':
        is_tool_result = True
        log_info("FunctionResponse", f"🔍 DEBUG: 通过类名检测到ToolResult对象")
    
    # 策略2: 通过isinstance检测（处理导入路径问题）
    if not is_tool_result:
        try:
            from ..types.tool_types import ToolResult
            if isinstance(llm_content, ToolResult):
                is_tool_result = True
                log_info("FunctionResponse", f"🔍 DEBUG: 通过isinstance检测到ToolResult对象")
        except ImportError:
            pass
    
    # 策略3: 通过属性检测（最灵活的方式）
    if not is_tool_result:
        # 检查是否具有ToolResult的所有特征属性
        has_summary = hasattr(llm_content, 'summary')
        has_llm_content = hasattr(llm_content, 'llm_content')
        has_return_display = hasattr(llm_content, 'return_display')
        has_error = hasattr(llm_content, 'error')
        
        # 如果有至少3个ToolResult特征属性，认为是ToolResult
        if sum([has_summary, has_llm_content, has_return_display, has_error]) >= 3:
            is_tool_result = True
            log_info("FunctionResponse", f"🔍 DEBUG: 通过属性检测到ToolResult对象")
    
    if is_tool_result:
        log_info("FunctionResponse", f"🔍 DEBUG: 确认为ToolResult对象，进入专用处理分支")
        # 处理 ToolResult 对象
        log_info("FunctionResponse", f"🔍 DEBUG: ToolResult属性检查:")
        log_info("FunctionResponse", f"🔍 DEBUG: - error: {repr(getattr(llm_content, 'error', None))}")
        log_info("FunctionResponse", f"🔍 DEBUG: - summary: {repr(getattr(llm_content, 'summary', None))}")
        log_info("FunctionResponse", f"🔍 DEBUG: - llm_content: {repr(str(getattr(llm_content, 'llm_content', ''))[:200])}")
        log_info("FunctionResponse", f"🔍 DEBUG: - return_display: {repr(str(getattr(llm_content, 'return_display', ''))[:200])}")
    ...
    if isinstance(content_to_process, list):
        ...
        try:
            ...
            function_response = create_function_response_part(
                call_id, tool_name, 'Tool execution completed with complex output.'
            )
    ...
    # 最后的回退
    return create_function_response_part(call_id, tool_name, 'Tool execution completed.')